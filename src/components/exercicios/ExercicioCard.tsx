...
import { MoreVertical, Copy, Edit, Trash2, Eye, Camera, Play } from "lucide-react";
...
export const ExercicioCard = ({ exercicio, onCriarCopia, onExcluir, isAdmin, location }: ExercicioCardProps) => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsVisible(true);
          observer.disconnect(); // Só dispara uma vez
        }
      },
      { threshold: 0.1 }
    );
  ...
  return (
    ...
    >
      {/* ✅ CONTEÚDO NO TOPO - NOME, MENU E BADGES */}
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full p-0 flex-shrink-0" onClick={(e) => e.stopPropagation()}><MoreVertical className="h-4 w-4" /></Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleNavigation(`/exercicios/detalhes/${exercicio.id}`); }}><Eye className="mr-2 h-4 w-4" />Ver Detalhes</DropdownMenuItem>
              {onCriarCopia && <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onCriarCopia(exercicio.id); }}><Copy className="mr-2 h-4 w-4" />Criar Cópia</DropdownMenuItem>}
              {exercicio.tipo === 'personalizado' && (
                <>
                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleNavigation(`/exercicios/editar/${exercicio.id}`); }}><Edit className="mr-2 h-4 w-4" />Editar</DropdownMenuItem>
                  {onExcluir && <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onExcluir(exercicio.id); }} className="text-destructive focus:text-destructive"><Trash2 className="mr-2 h-4 w-4" />Excluir</DropdownMenuItem>}
                </>
              )}
              {isAdmin && exercicio.tipo === 'padrao' && (
                <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleNavigation(`/exercicios/editar-padrao/${exercicio.id}`); }} className="bg-secondary text-secondary-foreground hover:bg-secondary/80 focus:bg-secondary/80 focus:text-secondary-foreground"><Edit className="mr-2 h-4 w-4" />Editar Padrão</DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        
        {/* ✅ BADGES NO TOPO (antes da imagem) */}
