  ...
  const handleSave = async (data: PostFormData, status: 'draft' | 'published') => {
    ...
    try {
      const uploadImage = async (image: ImageState, suffix: string): Promise<string | null> => {
        ...
        if (functionError || !uploadData?.success) {
          throw new Error(functionError?.message || uploadData.error || 'Falha ao obter URL de upload.');
        }
      ...
      const [desktopFileName, mobileFileName] = await Promise.all([
        uploadImage(desktopImage, 'desktop'),
        uploadImage(mobileImage, 'mobile'),
      ]);
      ...
      toast.error('Erro ao salvar post', { description: errorMessage });
    } finally {
      setIsSaving(false);
    }
  };
