...
const NovoExercicio = () => {
  ...
  useEffect(() => {
    ...
    setSignedUrls(prev => ({
      ...
      imagem3: midias.imagem_3_url ? (newSignedUrls.imagem3 || prev.imagem3) : undefined,
    ...
    <div className="space-y-6">
              ...
              <div className="mt-2 space-y-4">
                {midias.imagem_3_url ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button type="button" variant="outline" size="sm" onClick={() => signedUrls.imagem3 && window.open(signedUrls.imagem3, '_blank')} className="flex items-center gap-2" disabled={!signedUrls.imagem3 || saving}>
                        <Eye className="h-4 w-4" /> Ver
                      </Button>
                      <Button type="button" variant="outline" size="sm" onClick={() => handleSelectMedia('imagem3')} className="flex items-center gap-2" disabled={saving}>
                        {isMobile ? <Camera className="h-4 w-4" /> : <Upload className="h-4 w-4" />}
                        {isMobile ? 'Nova Foto' : 'Alterar'}
                      </Button>
                      <Button type="button" variant="outline" size="sm" onClick={() => setShowDeleteMediaDialog('imagem3')} className="flex items-center gap-2" disabled={saving}>
                        <Trash2 className="h-4 w-4" /> Excluir
                      </Button>
                    </div>
                  </div>
      ...
      <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 z-50">
        ...
        >
          ...
          <span className="sr-only">{saving ? "Salvando..." : "Salvar Exercício"}</span>
        </Button>
      </div>
    </div>
  );
